// Package poseexecutor implements a generic service that moves an arm
// through the poses generated by an image-to-poses camera.
package poseexecutor
...
var Model = resource.NewModel("esha", "portrait-drawing", "pose-executor")

func init() {
	resource.RegisterService(generic.API, Model, resource.Registration[resource.Resource, *Config]{
		Constructor: newPoseExecutor,
	})
}
...
func (s *poseExecutor) DoCommand(ctx context.Context, cmd map[string]interface{}) (map[string]interface{}, error) {
...
func (s *poseExecutor) Close(ctx context.Context) error {
	s.mu.Lock()
	cancel := s.drawCancel
	s.mu.Unlock()
	if cancel != nil {
...
func (s *poseExecutor) fetchPoses(ctx context.Context) ([]spatialmath.Pose, error) {
